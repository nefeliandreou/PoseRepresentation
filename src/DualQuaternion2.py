...
class DualQuaternion(object):
    ...
    def from_dq_array(cls, r_wxyz_t_wxyz):
        ...
        """
        #todo
        return cls(Quaternion(*r_wxyz_t_wxyz[:4]), Quaternion(*r_wxyz_t_wxyz[4:]), normalize=True)
    ...
    def from_homogeneous_matrix(cls, arr):
        ...
        """
        q_r = quaternion.from_rotation_matrix(arr[:3, :3]).normalized()
    ...
    def from_quat_pose_array(cls, r_wxyz_t_xyz):
        ...
        """
        q_r = Quaternion(*r_wxyz_t_xyz[:4]).normalized()
    ...
    def quaternion_conjugate(self):
        ...
        This is equivalent to inverse of a homogeneous matrix. It is  in applying
    ...
    def is_normalized(self):
        ...
        if np.isclose(np.sqrt(self.q_r.norm()), 0):
            return True
        rot_normalized = np.isclose(np.sqrt(self.q_r.norm()), 1)
        trans_normalized = (self.q_d / np.sqrt(self.q_r.norm())) == self.q_d
    ...
    def pow(self, exponent):
        ...
        q_d = Quaternion(scalar=-exponent * d / 2 * np.sin(exponent * theta / 2),
                         vector=exponent * d / 2 * np.cos(exponent * theta / 2) * s0 + np.sin(
                             exponent * theta / 2) * se)
    ...
    def homogeneous_matrix(self):
        ...
        homogeneous_mat[:3, :3] = quaternion.as_rotation_matrix(self.q_r)
    ...
    def translation(self):
        ...
        :return: list [x y z]
        """
        mult = (2.0 * self.q_d) * self.q_r.normalized().conjugate()

        return [mult.x, mult.y, mult.z]

    def normalized(self):
        """Return a copy of the normalized dual quaternion"""
        norm_qr = np.sqrt(self.q_r.norm())
        #         print(norm_qr)
    ...
    def crew(cls, l, m, theta, d):
        ...
        q_d = Quaternion(scalar=-d / 2 * np.sin(theta / 2),
                         vector=np.sin(theta / 2) * m + d / 2 * np.cos(theta / 2) * l)
